...
// ════════════════════════════════════════════════════════════════════
function GameDetail({ game, friends, news, onPlay, onInstall, onUpdate, onWallet }) {
    ...
    <div className="scroll" style={{flex:1, display:"grid", gridTemplateColumns: "1fr 280px", gridTemplateRows: "auto 1fr", gap: 18, padding: 20, position:"relative"}}>
      ...
      <div style={{
        ...
        <div style={{
          position:"absolute", inset: 0,
          background:
            "linear-gradient(180deg, transparent 0%, transparent 35%, rgba(11,6,24,.6) 70%, rgba(11,6,24,.95) 100%)," +
            "linear-gradient(90deg, rgba(11,6,24,.85) 0%, rgba(11,6,24,.4) 35%, transparent 60%)",
        }} />
        <div className="scanlines" style={{opacity:.18}} />
          ...
          <div style={{display:"flex", alignItems:"center", gap: 14}}>
            ...
            {status === "update" && (
              <div style={{display:"flex", flexDirection:"column", gap: 4}}>
                ...
                <span className="mono" style={{fontSize: 14, color:"var(--pix-ink-3)"}}>
                  v{game.version} → v{game.nextVersion} · 84 MB
                </span>
              </div>
            )}
            {status === "install" && (
      ...
      <div style={{display:"flex", flexDirection:"column", gap: 18, minWidth: 0}}>
        ...
        <div className="panel" style={{padding: 14}}>
          ...
          <div style={{display:"flex", alignItems:"center", gap: 12, marginBottom: 12}}>
            <div style={{
              width: 56, height: 56,
              border:"2px solid var(--pix-magenta)",
              background:"radial-gradient(circle at 50% 60%, #6b0c3a, #1a0612)",
              display:"grid", placeItems:"center",
              boxShadow:"0 0 14px rgba(255,30,133,.4)",
            }}>
              <PixenAvatar seed={0} scale={4} />
            </div>
            <div>
              <div className="pix" style={{fontSize:10, color:"var(--pix-ink)", marginBottom: 4, letterSpacing:".08em"}}>
                PIXEN #1
              </div>
              <div className="eyebrow lime" style={{fontSize:7}}>UNCOMMON · LV 1</div>
            </div>
          </div>
          <button className="btn ghost sm block" onClick={onWallet}>SWITCH PIXEN →</button>
        </div>
        ...
        <div className="panel" style={{flex: 1, minHeight: 0, display:"flex", flexDirection:"column"}}>
          <div className="panel-hd">
            <h3>FRIENDS</h3>
            <span className="meta mono">{friends.filter(f=>f.online).length} ONLINE</span>
          </div>
          <div className="scroll" style={{flex:1, padding: "10px 8px"}}>
            {friends.map((f, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  position:"relative",
                  width:32, height:32,
                  background: `radial-gradient(circle at 50% 60%, ${f.bg}, #0e041e)`,
                  display:"grid", placeItems:"center",
                }}>
                  <PixenAvatar seed={f.seed} scale={2} />
                  <span style={{
                    position:"absolute", bottom:-2, right:-2,
                    width:10, height:10,
                    background: f.online ? "var(--pix-lime)" : "var(--pix-ink-4)",
                    border: "2px solid var(--pix-bg)",
                    boxShadow: f.online ? "0 0 6px var(--pix-lime)" : "none",
                  }} />
                </div>
                <div style={{flex:1, minWidth:0}}>
                  <div className="mono" style={{fontSize: 15, color: f.online ? "var(--pix-ink)" : "var(--pix-ink-4)", lineHeight:1.1}}>{f.name}</div>
                  <div className="mono" style={{fontSize: 13, color:"var(--pix-ink-4)", lineHeight:1.2, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis"}}>
                    {f.status}
                  </div>
                </div>
              </div>
          ...
          <div style={{padding: 10, borderTop:"1px solid var(--pix-line)", display:"flex", gap:6}}>
            <input className="input" placeholder="add by 0x..." style={{height: 32, fontSize:14}} />
      ...
      <div style={{display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap: 16}}>
        {games.map(g => (
          <button key={g.id} onClick={()=>onPick(g.id)} style={{
            background:"transparent", border: "1px solid var(--pix-line-2)",
            ...
            <div style={{
              ...
              background: g.cover
                ...
                : "linear-gradient(135deg, #1a0830, #3a0f6a)",
              position:"relative",
              imageRendering: "pixelated",
            }}>
              {!g.cover && (
                <div style={{position:"absolute", inset:0, display:"grid", placeItems:"center"}}>
                  <PixenSprite color="#ff1e85" scale={6} />
                </div>
              )}
              <div style={{position:"absolute", top: 8, left: 8}}>
                <span className={`eyebrow ${g.tagColor}`} style={{
                  background:"rgba(11,6,24,.75)", padding:"4px 6px",
                }}><span className="dot" />{g.kind}</span>
              </div>
              {g.hasUpdate && (
                <div style={{position:"absolute", top: 8, right: 8,
                  background: "var(--pix-cyan)", color:"#061018",
                  fontFamily:"var(--font-pixel)", fontSize: 7, padding:"3px 5px",
                }}>UPDATE</div>
              )}
            </div>
            <div style={{padding: 14}}>
              <div className="pix" style={{fontSize: 11, color:"var(--pix-ink)", marginBottom:6, letterSpacing:".06em"}}>
                {g.titleParts.map((p, i) => <span key={i} className={p.cls}>{p.t}</span>)}
              </div>
              <div className="mono" style={{fontSize: 14, color:"var(--pix-ink-3)", lineHeight: 1.3}}>
                {g.desc}
              </div>
              <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:10}}>
                <span className={`eyebrow ${g.status === "installed" ? "lime" : g.status === "update" ? "cyan" : "magenta"}`} style={{fontSize:7}}>
                  <span className="dot" />
                  {g.status === "installed" ? "READY"
                    : g.status === "update" ? "UPDATE"
                    : g.status === "playing" ? "RUNNING"
                    : "NOT INSTALLED"}
                </span>
                <span className="mono" style={{fontSize:13, color:"var(--pix-ink-4)"}}>{g.size}</span>
              </div>
            </div>
          </button>
        ))}
...
function UpdateScreen({ game, onClose }) {
  const [progress, setProgress] = useState(0);
  const [phase, setPhase] = useState("DOWNLOAD"); // DOWNLOAD | VERIFY | INSTALL | DONE

  useEffect(() => {
    const t = setInterval(() => {
      setProgress(p => {
        if (p >= 100) {
          if (phase === "DOWNLOAD") { setPhase("VERIFY"); return 0; }
          if (phase === "VERIFY")   { setPhase("INSTALL"); return 0; }
          if (phase === "INSTALL")  { setPhase("DONE"); clearInterval(t); return 100; }
        }
        return Math.min(100, p + (phase === "VERIFY" ? 8 : 3));
      });
    }, 180);
    return () => clearInterval(t);
  }, [phase]);
  ...
  return (
    <div className="scroll" style={{flex:1, padding: 28, display:"grid", gridTemplateColumns:"1fr 1fr", gap: 24}}>
      <div>
        <div className={`eyebrow ${phase === "DONE" ? "lime" : "cyan"}`} style={{marginBottom: 10}}><span className="dot" />
          {phase === "DONE" ? "UPDATE COMPLETE" : "UPDATING"}
        </div>
        ...
        <div style={{display:"flex", justifyContent:"space-between", marginBottom: 8}}>
          <span className="pix" style={{fontSize: 9, color:"var(--pix-cyan)", letterSpacing:".14em"}}>
            {phase === "DONE" ? "✓ READY TO PLAY" : `▸ ${phase}…`}
          </span>
          <span className="mono" style={{fontSize: 16, color:"var(--pix-cyan)"}}>
            {phase === "DONE" ? "100%" : `${Math.floor(progress)}%`}
          </span>
        </div>
        <div className="progress cyan" style={{height: 20}}>
          <div className="bar" style={{width: phase === "DONE" ? "100%" : `${progress}%`}} />
        </div>

        {/* Phase steps */}
        <div style={{display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap: 0, marginTop: 18}}>
          {["DOWNLOAD","VERIFY","INSTALL","DONE"].map((p, i) => {
            const phases = ["DOWNLOAD","VERIFY","INSTALL","DONE"];
            const cur = phases.indexOf(phase);
            const done = i < cur || phase === "DONE";
        ...
        <div style={{display:"flex", gap: 12, marginTop: 26}}>
          <button className="btn ghost" onClick={onClose}>{phase === "DONE" ? "BACK" : "PAUSE"}</button>
          {phase === "DONE" && <button className="btn magenta">▸ PLAY NOW</button>}
        </div>

        <div className="panel" style={{padding: 16, marginTop: 26}}>
          <div className="eyebrow" style={{marginBottom: 8}}><span className="dot" />DOWNLOAD STATS</div>
          <div style={{display:"grid", gridTemplateColumns:"repeat(2, 1fr)", gap:14}}>
            <Stat label="SPEED" value={phase === "DONE" ? "—" : "42.3 MB/s"} col="cyan" />
            <Stat label="ETA" value={phase === "DONE" ? "—" : "00:18"} col="cyan" />
            <Stat label="PEERS" value="14" col="ink" />
            <Stat label="REGION" value="EU-WEST" col="ink" />
          </div>
        </div>
      </div>
      ...
      <div className="panel" style={{display:"flex", flexDirection:"column", minHeight: 0}}>
        <div className="panel-hd">
          <h3>CHANGELOG · v{game.nextVersion}</h3>
          <span className="meta mono">just now</span>
        </div>
        <div className="scroll" style={{padding: 18, flex:1, fontFamily:"var(--font-mono)", fontSize:15, color:"var(--pix-ink-2)", lineHeight:1.5}}>
          <div className="eyebrow magenta" style={{marginBottom: 6}}>HIGHLIGHTS</div>
          <ul style={{listStyle:"none", marginBottom: 16}}>
            <li>▸ New Hero Pixen <span style={{color:"var(--pix-yellow)"}}>WARMEC</span> joins the roster</li>
            <li>▸ Map #11 <span style={{color:"var(--pix-cyan)"}}>NEXUS CORE</span> — rotating event maps</li>
            <li>▸ Spell rebalance: <span style={{color:"var(--pix-magenta)"}}>Garbage Freeze</span> cooldown 8s → 6s</li>
          </ul>
          <div className="eyebrow cyan" style={{marginBottom: 6}}>FIXES</div>
          <ul style={{listStyle:"none", marginBottom: 16}}>
            <li>▸ Wallet auto-disconnect on alt-tab</li>
            <li>▸ Drones occasionally walked through walls</li>
            <li>▸ Audio stutter on AMD GPUs (driver 24.x)</li>
          </ul>
          <div className="eyebrow lime" style={{marginBottom: 6}}>ECON</div>
          <ul style={{listStyle:"none"}}>
            <li>▸ Daily quest rewards +25% diamonds</li>
            <li>▸ Genesis spell-charge bonus restored</li>
          </ul>
        </div>
      </div>
    </div>
...
function Stat({ label, value, col }) {
  return (
    <div>
      <div className="eyebrow" style={{fontSize:7, color:"var(--pix-ink-4)", marginBottom: 4}}>{label}</div>
      <div className="mono" style={{fontSize: 18, color: col === "cyan" ? "var(--pix-cyan)" : "var(--pix-ink)"}}>{value}</div>
    </div>
  );
}
...
Object.assign(window, {
  ...
  WalletScreen, SettingsScreen, UpdateScreen,
});
