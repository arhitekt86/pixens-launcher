  ...
  return (
    <div className="titlebar">
      ...
      <div className="tb-btns">
        <button className="tb-btn" title="Minimize">─</button>
        <button className="tb-btn" title="Maximize">▢</button>
      ...
      <div className="tb-right">
        {loggedIn ? (
          <>
            ...
            <div className="tb-pill" title="BNB Wallet">
              <span style={{color:"var(--pix-yellow)"}}>◆</span>
              <span className="v">0.227</span>
            </div>
            <div className="tb-pill purple" title="Connected wallet">
              <span style={{color:"var(--pix-purple)"}}>◇</span>
              <span className="v">0xa487</span>
            </div>
            <button className="tb-icon-btn" title="Notifications">
              <span style={{fontSize:18, lineHeight:0}}>♪</span>
              <span className="badge">3</span>
            </button>
            <button className="tb-icon-btn" title="Friends">
              <span style={{fontSize:14}}>♥</span>
            </button>
            <div className="tb-user" onClick={onAvatar} style={{cursor:"pointer"}}>
              <div className="av"><PixenAvatar seed={4} scale={2} /></div>
              <div style={{display:"flex", flexDirection:"column", lineHeight:1}}>
                <span className="nm">{user?.name || "IAMPIXEN"}</span>
                <span className="lv">LV {user?.level || 1}</span>
              </div>
              <span style={{color:"var(--pix-ink-4)", marginLeft:4, fontSize:11}}>▾</span>
            </div>
          </>
        ) : (
...
function Sidebar({ games, activeGameId, onPick, onSettings, onAll }) {
  return (
